...
These skills work together as a pipeline: research, ideation, creation, audit, optimization.
...
5. **Audits the draft** — runs the Content Audit and revises up to three times against a 40+ point E-E-A-T checklist
6. **Adds illustrations** — identifies 1-3 sections that benefit from a visual and generates image prompts
...
## Scenario F: Audit an existing post

If you have a blog already published — or a draft written outside the blog creator — you can still check it against the quality ruleset.

Run the [Content Audit](/content-audit) on a URL or a local file. It returns a scored report across seven dimensions (Clarity, Organization, Rigor, Exclusivity, Experience, Expertise, Trust) with specific findings you can act on.

This is the same audit that runs inside `/blog-creator`, just exposed on its own so you can use it against content you didn't write with RankEarly.

## Scenario G: Add a cover image or illustration
...
4. **Audit for quality.** The blog creator runs the Content Audit automatically after writing. It checks the draft against a 40+ point E-E-A-T checklist and revises up to three times. If anything remains unresolved, it flags it for you.

5. **Review and refine.** The finished blog and illustrations are saved to `blogs/<topic>/`. Read through and make any adjustments.

6. **Optimize the title.** Run `/blog-title-generator` on the finished blog to get 10 scored title options.

7. **Add a cover image.** Run `/blog-image` with your chosen title to generate a cover image prompt.
